...
mongoose.connect(`mongodb+srv://${process.env.DB_USERNAME}:${process.env.DB_KEY}@cluster0.gqlzq.mongodb.net/mernchat?retryWrites=true&w=majority`, {
        ...
        Room.findOne({ roomname: req.params.roomName, password: req.params.roomPassword }, (err, message) => {
            if (message) {
                ...
                if (err) {
                    ...
                    res.render("result", { message: "Couldn't Find the Room 🤷‍♀️" });
                }
            }
        });
        ...
        Room.findOne({ roomname: roomname, password: password }, (err, foundRoom) => {
            if (foundRoom) {
                ...
                if (err) {
                    ...
                    res.render("result", { message: "Check The Credentials !🤦‍♀️🤦‍♂️" });
                }
            }
        });
...
app.post("/delete-room", (req, res) => {
    ...
    Room.deleteOne({ roomname: roomName, password: roomPassword }, (err) => {
        if (err) {
            ...
            res.render("result", { message: "ROOM IS DELETED SUCCESSGULLY" });
        }
    })
})
    ...
    Room.findOne({ roomname: roomName, password: roomPassword }, (err, message) => {
        if (message) {
            ...
            if (err) {
                ...
                res.render("result", { message: "ROOM NOT FOUND 404" });
            }
        }
    });
});
